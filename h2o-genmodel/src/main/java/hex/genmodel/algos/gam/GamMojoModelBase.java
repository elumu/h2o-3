...
public abstract class GamMojoModelBase extends MojoModel implements ConverterFactoryProvidingModel {
  ...
  int _lastClass;
  int _predictColumnNum; // Count number of columns that are found both in input data row and gam predictor columns
  
  public int getTotFeatureSize() { return _totFeatureSize;}
  public String[] getNames() { return _names; }  // get column names centered
  public int getPredictColumnNum() { return _predictColumnNum;}
  
  GamMojoModelBase(String[] columns, String[][] domains, String responseColumn) {
    super(columns, domains, responseColumn);
  }

  public void setPredictColumNum(String[] inputColumnNames) {
    _predictColumnNum = 0;
     for (String cName : _names) {
      if (Arrays.asList(inputColumnNames).contains(cName))
        _predictColumnNum++; // count column names that are present both in input data row and gam predictor names
    }
  }
  ...
  double evalLink(double val, LinkFunctionType linkType) {
    switch (linkType) {
